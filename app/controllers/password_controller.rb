class PasswordController < ApplicationController
    ...
    unless @user and @user.expires_at > Time.now
      redirect_to login_url, flash[:alert] = "Reset link has expired. Please try again."
    end
  end
  ...
  def update
    ...
    if @user and @user.expires_at > Time.now
      ...
      if @user.save
        ...
        redirect_to root_url, flash[:alert] = "Password changed successfully!"
  ...
  def user_params
    params.require(:user).permit(:password, :password_confirmation, :code, :expires_at)
  end
end
